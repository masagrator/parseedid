...
float parseEdid(unsigned char* edid) {
	...
	for (size_t i = 0; i < 2; i++) {
		...
		uint16_t widthBackPorch = (uint16_t)(((uint16_t)td.horizontal_blanking_pixels_msb << 8 | td.horizontal_blanking_pixels_lsb) - (widthSync + widthFrontPorch));
		uint16_t heightBackPorch = (uint16_t)(((uint16_t)td.vertical_blanking_lines_msb << 8 | td.vertical_blanking_lines_lsb) - (heightSync + heightFrontPorch));
		*/
	...
	if (extension_type != 2) {
		memcpy(&edid[offsetof(SetSysEdid, extension_tag)], &edid[offsetof(SetSysEdid, data2)], 0x80);
		memcpy(&extension_type, &edid[offsetof(SetSysEdid, extension_tag)], 1);
		if (extension_type != 2) {
			memcpy(&edid[offsetof(SetSysEdid, extension_tag)], &edid[offsetof(SetSysEdid, data3)], 0x80);
			memcpy(&extension_type, &edid[offsetof(SetSysEdid, extension_tag)], 1);			
		}
		if (extension_type != 2) {
			#ifdef DEBUG
			printf("No valid extension type detected!\n");
			#endif
			return highestRefreshRate;
		}
	}
	...
	if (dtd_start) {
		...
		if (data -> video.block_type == SetSysBlockType_Video) {
			printf("SetSysBlockType_Video size: %d\n", data -> video.size);
		...
		for (size_t i = 0; i < 5; i++) {
			...
			*/
			...
			float refreshRate = ((float)(td.pixel_clock * 10000) / (float)(h_total * v_total));
			#ifdef DEBUG
			if (td.pixel_clock) printf("Res: %dx%d%s, pixel clock: %d, refresh rate: %0.4f\n", width, height, (td.interlaced ? "i" : ""), td.pixel_clock * 10, refreshRate);
			#endif
